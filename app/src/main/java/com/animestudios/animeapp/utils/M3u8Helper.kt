...
object M3u8Helper2 {
    ...
    private fun selectBest(qualities: List<M3u8Helper.M3u8Stream>): M3u8Helper.M3u8Stream? {
        val result = qualities.sortedBy {
            it.quality
                 //if (it.quality != null && it.quality <= 1080)  else 0
        }/*.filter {
    ...
    suspend fun m3u8Generation(
        ...
        val response = com.animestudios.animeapp.tools.client.get(m3u8.streamUrl, headers = m3u8.headers, verify = false).text
    ...
    data class LazyHlsDownloadData(
        ...
        suspend fun resolveLink(index: Int): ByteArray {
            ...
            val tsResponse = client.get(ts.url, headers = headers, verify = false)
            val tsData = tsResponse.body.bytes()
            if (tsData.isEmpty()) throw Exception("no data")
    ...
    suspend fun hslLazy(
        ...
        val selected = if (selectBest) {
            selectBest(qualities)
        } else {
            selectWorst(qualities)
        } ?: qualities.first()
        ...
        val secondSelection = if (selectBest) {
            selectBest(innerStreams)
        } else {
            selectWorst(innerStreams)
        }
        ...
        if (encryptionState) {
            ...
            val encryptionKeyResponse = client.get(encryptionUri, headers = headers, verify = false)

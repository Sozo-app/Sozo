...
import com.animestudios.animeapp.tools.getAttrValue
...
object MarkdownUtil {
    ...
    fun getMarkdownSetup(
        context: Context,
        maxWidth: Int,
        linkClickAction: ((String) -> Unit)?
    ): MarkdownSetup {
    ...
    private fun replaceMarkdownText(markdownText: String): String {
        ...
        val rogueUrlRegex =
            "((?<=\\s)|^)(http|ftp|https):\\/\\/([\\w_-]+(?:(?:\\.[\\w_-]+)+))([\\w.,@?^=%&:\\/~+#-]*[\\w@?^=%&\\/~+#-])?".toRegex()
...
object Constant {
    ...
    const val ALCHAN_YOUTUBE_THUMBNAIL_URL =
        "${ALCHAN_RAW_GITHUB_URL}docs/images/youtube_thumbnail.png"

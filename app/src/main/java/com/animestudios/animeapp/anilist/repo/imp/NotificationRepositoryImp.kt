...
import com.animestudios.animeapp.anilist.response.PageInfo
...
class NotificationRepositoryImp @Inject constructor(private val client: AniListClient) :
    ...
    suspend fun getNotifications(page: Int): Pair<NotificationsQuery.PageInfo?, List<AniNotification>?> =
        withContext(Dispatchers.IO) {
            val response =
                client.getNotifications(page) ?: return@withContext Pair(null, emptyList())
            ...
            val notifications = notificationsData!!.mapNotNull { notification ->

...
import com.animestudios.animeapp.databinding.ActivityExoplayerBinding
import com.animestudios.animeapp.media.Media
import com.animestudios.animeapp.others.AniSkip
import com.animestudios.animeapp.others.AniSkip.getType
import com.animestudios.animeapp.others.ResettableTimer
import com.animestudios.animeapp.others.VideoCache
import com.animestudios.animeapp.others.getSerialized
import com.animestudios.animeapp.parsers.*
import com.animestudios.animeapp.settings.PlayerSettings
...
class ExoplayerView : AppCompatActivity(), Player.Listener {
    ...
    private lateinit var binding: com.animestudios.animeapp.databinding.ActivityExoplayerBinding
    ...
    val model: DetailsViewModelImpl by viewModels()
    ...
    private fun setupSubFormatting(playerView: PlayerView, settings: PlayerSettings) {
        ...
        val outline = when (settings.outline) {
            0    -> CaptionStyleCompat.EDGE_TYPE_OUTLINE // Normal
            1    -> CaptionStyleCompat.EDGE_TYPE_DEPRESSED // Shine
            2    -> CaptionStyleCompat.EDGE_TYPE_DROP_SHADOW // Drop shadow
            3    -> CaptionStyleCompat.EDGE_TYPE_NONE // No outline
            else -> CaptionStyleCompat.EDGE_TYPE_OUTLINE // Normal
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        settings = readData("player_settings") ?: PlayerSettings().apply { saveData("player_settings", this) }
        uiSettings = readData("ui_settings") ?: UISettings().apply { saveData("ui_settings", this) }
        ...
        keyMap[KeyEvent.KEYCODE_DPAD_RIGHT] = { seek(true) }
        keyMap[KeyEvent.KEYCODE_DPAD_LEFT] = { seek(false) }
        ...
        model.getEpisode().observe(this) {
            ...
            if (it != null && !epChanging) {
                ...
                playbackPosition = readData("${media.id}_${it.number}", this) ?: 0
                initPlayer()
                preloading = false
                updateProgress()
            }
        ...
        //FullScreen
        isFullscreen = readData("${media.id}_fullscreenInt", this) ?: isFullscreen
...
//            startActivity(intent)
        ...
        var curSpeed = readData("${media.id}_speed", this) ?: settings.defaultSpeed
        ...
        preloading = false
        val showProgressDialog = if (settings.askIndividual) readData<Boolean>("${media.id}_progressDialog") ?: true else false
    ...
    private fun initPlayer() {
        ...
        val set = readData<MutableSet<Int>>("continue_ANIME", this) ?: mutableSetOf()
        ...
        subtitle = intent.getSerialized("subtitle")
            ?: when (val subLang: String? = readData("subLang_${media.id}", this)) {
                null   -> {
        ...
        if (video?.format == VideoType.CONTAINER || (readData<Int>("settings_download_manager") ?: 0) != 0) {
...
//                download(this, episode, animeTitle.text.toString())
        ...
        trackSelector.setParameters(
            trackSelector.buildUponParameters()
                .setMinVideoSize(readData("maxWidth", this) ?: 720, readData("maxHeight", this) ?: 480)
                .setMaxVideoSize(1, 1)
        )
    ...
    private fun buildExoplayer() {
        ...
        exoPlayer = ExoPlayer.Builder(this)
            ...
            .build().apply {
                ...
                prepare()
                readData<Long>("${media.id}_${media.anime!!.selectedEpisode}_max")?.apply {
...
//        SubtitleDialogFragment().show(supportFragmentManager, "dialog")
    ...
    private fun initPopupQuality(): Dialog {
        ...
        val trackSelectionDialogBuilder =
            TrackSelectionDialogBuilder(this, "Available Qualities", exoPlayer, C.TRACK_TYPE_VIDEO)
    ...
    private fun updateAniProgress() {
        if (exoPlayer.currentPosition / episodeLength > settings.watchPercentage && Anilist.userid != null)
            if (readData<Boolean>("${media.id}_save_progress") != false && if (media.isAdult) settings.updateForH else true) {
    ...
    private val keyMap: MutableMap<Int, (() -> Unit)?> = mutableMapOf(
        ...
        KeyEvent.KEYCODE_N to { exoNext.performClick() },
        KeyEvent.KEYCODE_B to { exoPrev.performClick() }
    )

...
import com.animestudios.animeapp.utils.Coroutines.ioSafe
import com.animestudios.animeapp.utils.Globals.TV
import com.animestudios.animeapp.utils.Globals.isLayout
import com.animestudios.animeapp.utils.M3u8Helper
import com.animestudios.animeapp.utils.M3u8Helper2
...
interface IPreviewGenerator {
    ...
    companion object {
        fun new(): IPreviewGenerator {
            val userDisabled = App.currentContext()?.let { ctx ->
...
class PreviewGenerator : IPreviewGenerator {
    ...
    fun load(link: FileUrl, keepCache: Boolean) {
        clear(keepCache)

//        when (link.type) {
//            ExtractorLinkType.M3U8 -> {
        currentGenerator = M3u8PreviewGenerator(params).apply {
            load(url = link.url, headers = link.headers)
        }
//            }
//
...
//        }
    ...
    fun load(context: Context, link: FileUrl, keepCache: Boolean) {
        clear(keepCache)
        currentGenerator = Mp4PreviewGenerator(params).apply {
            load(keepCache = keepCache, context = context, uri = Uri.parse(link.url))
        }
    }
}
